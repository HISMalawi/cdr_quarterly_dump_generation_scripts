...
#####################################################################################################
quarter_name=$(mysql --host=127.0.0.1  --port=3305 -u$username -p$password -D  $db -se "SELECT CONCAT('Q', QUARTER(DATE_SUB(NOW(), INTERVAL 3 MONTH)),'_',YEAR(DATE_SUB(NOW(), INTERVAL 3 MONTH)))")

q_object=$(mysql --host=127.0.0.1 --port=3305  -u$username -p$password -D $db  -se "select concat(replace(replace(replace(replace(replace(lower(name),' ','_'),')',''),'(',''),'''',''),'/',''), '_', location_id, 'general_stats_rq_', '$quarter_name') site_name from $db.location l where location_id in (select property_value from $db.global_property gp where lower(property)= 'current_health_center_id')")

echo "$q_object to be created in the next step"

mysql --host=127.0.0.1 --port=3305 -u$username -p$password $db -se "drop table if exists $q_object"

mysql --host=127.0.0.1 --port=3305 -u$username -p$password $db -se "create table if not exists $q_object (table_name text null, row_counts text null, max_recorded_date text null )"
#########################################################################################
...
do
  if ! mysql --host=127.0.0.1 --port=3305  -u$username -p$password -e "use $db; describe $table" &> /dev/null; then
...
do
    ...
    record_counts=$(mysql --host=127.0.0.1 --port=3305  -u$username -p$password -D $db -se "select count(*) from $table")

    # Execute query to get max recorded date based on table
    if [ $table = "obs" ]; then
        max_recorded_date=$(mysql --host=127.0.0.1 --port=3305  -u$username -p$password -D $db -se "select max(cast(value_datetime as date)) from obs")
    elif [ $table = "encounter" ]; then
        max_recorded_date=$(mysql --host=127.0.0.1 --port=3305  -u$username -p$password -D $db -se "select max(cast(encounter_datetime as date)) from encounter")
    elif [ $table = "orders" ]; then
        max_recorded_date=$(mysql --host=127.0.0.1 --port=3305  -u$username -p$password -D $db -se "select max(cast(date_created as date)) from orders")
    else
        max_recorded_date='NULL'
    fi

    # Insert values into q_objects table
    mysql --host=127.0.0.1 --port=3305  -u$username -p$password -D $db -se "INSERT INTO $q_object(table_name, row_counts, max_recorded_date) VALUES ('$table_name', '$record_counts','$max_recorded_date')"
done

# Use the "$username" and "$password" variables as needed for further operations.

name=$(mysql --host=127.0.0.1 --port=3305  -u$username -p$password $db -se "select concat(location_id,'_',replace(replace(replace(lower(name),' ','_'),')',''),'(','')) name from $db.location l where location_id in (select property_value from $db.global_property gp where lower(property)='current_health_center_id')")
